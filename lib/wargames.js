var io = require('socket.io'),
	...
	learndb = require('./learndb'),
	BotRSS = require('./botrss');
...
function Wargames(app, options) {
	...
	self.settings = {
		...
		ldbname: options.ldbname,
		ldbtable: options.ldbtable,
		ldbuser: options.ldbuser,
		ldbpass: options.ldbpass,
		bitlyUser: options.bitlyUser,
		bitlyKey: options.bitlyKey,
		rssGit: options.rssGit,
		rssTwit: options.rssTwit,
		rssCuss: options.rssCuss
	};
	...
	self.bitly = new Bitly(self.settings.bitlyUser, self.settings.bitlyKey);

	self.initiateCaching(function () {
		self.bot = self.initiateIrcBot();
		self.server = self.initiateWebsocketServer();
		new learndb(self.bot, self.settings);
		self.initiateRSS();
...
Wargames.prototype.initiateRSS = function() {
	var self=this;

	/* twitter cursing, used for testing */
	if(self.settings.rssCuss != undefined) {
		self.settings.rssCuss.lastPub = new Date().getTime();
		var cussFeed = new BotRSS(self.settings.rssCuss);
		cussFeed.addListener('rssitem', function(item) {
			pubdate = new Date(item.pubDate).getTime();
			if (pubdate > self.settings.rssCuss.lastPub) {
				newmsg = '['+self.settings.rssCuss.name+'] ' + item.title;
				console.log(newmsg);
				self.bot.say(self.settings.ircChannel, newmsg);
				self.settings.rssCuss.lastPub = pubdate;
			}
		});
	}

	/* github */
	if(self.settings.rssGit != undefined) {
		self.settings.rssGit.lastPub = new Date().getTime();
		var gitFeed = new BotRSS(self.settings.rssGit);
		gitFeed.addListener('rssitem', function(item) {
			pubdate = new Date(item.pubDate).getTime();
			if (pubdate > self.settings.rssGit.lastPub) {
				self.bitly.shorten(item.link, function(result) {
					newmsg = '['+self.settings.rssGit.name+'] '
								+ item.title + ' :: ' + result.data.url;
					console.log(newmsg);
					self.bot.say(self.settings.ircChannel, newmsg);
					self.settings.rssGit.lastPub = pubdate;
				});
			}
		});
	}
	...
	/* twitter */
	if(self.settings.rssTwit != undefined) {
		self.settings.rssTwit.lastPub = new Date().getTime();
		var twitFeed = new BotRSS(self.settings.rssTwit);
		twitFeed.addListener('rssitem', function(item) {
			pubdate = new Date(item.pubDate).getTime();
			if (pubdate > self.settings.rssTwit.lastPub) {
				newmsg = '['+self.settings.rssTwit.name+'] ' + item.title;
				console.log(newmsg);
				self.bot.say(self.settings.ircChannel, newmsg);
				self.settings.rssTwit.lastPub = pubdate;
			}
		});
	}
}
