...
function Wargames(app, options) {
	...
	self.settings = {
		...
		rssCounter: 0, //will be set during cache load
		rssInterval: options.rssInterval,
	...
	self.initiateCaching(function () {
		...
		setInterval(function () {
			botrss.doRSS(self.bot, self.settings, self.bitly); },
			self.settings.rssInterval);
	});
};
...
Wargames.prototype.initiateCaching = function (callback) {
	...
	fs.readFile(self.settings.cachePath, function (err, data) {
		...
		self.settings.rssCounter = cacheData.rssCounter || new Date().getTime();
		...
		setInterval(function () {
			var cache = {
				...
				messages: self.messages,
				messageCount: self.messageCount,
				rssCounter: self.settings.rssCounter
			};
